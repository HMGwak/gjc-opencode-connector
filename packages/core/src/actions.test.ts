...
describe("PendingActionService", () => {
  test("rejects stale optimistic versions without changing the pending action", () => {
    ...
    service.create({ payload: { prompt: "Continue?" }, expiresAt: expiry });
  ...
  test("makes an identical duplicate response idempotent but rejects a conflict", () => {
    const service = serviceAt("2030-01-01T00:00:00.000Z");
    service.create({ payload: {}, expiresAt: expiry });
  ...
  test("accepts before expiry and expires at and after the boundary", () => {
    ...
    serviceAt("2030-01-01T00:00:00.999Z", beforeDb).create({ payload: {}, expiresAt: expiry });
    ...
    const atDb = database();
    serviceAt("2030-01-01T00:00:00.000Z", atDb).create({ payload: {}, expiresAt: expiry });
    ...
    serviceAt("2030-01-01T00:00:00.000Z", afterDb).create({ payload: {}, expiresAt: expiry });
  ...
  test("refuses cancelled, expired, unknown, and missing actions", () => {
    ...
    unknown.create({ payload: {}, expiresAt: expiry });
  ...
  test("rolls back action creation and audit write together on a simulated crash", () => {
    ...
    expect(() => db.transaction(() => {
      db.createPendingAction({ id: "action-1", payload: {}, expiresAt: expiry });
